...
//
// Nota (209 / auditoría H21): ni la edición inline del catálogo ni "+
// Nuevo producto" validaban que precio/cantidad estándar no fueran
// negativos — el `min="0"` del HTML es solo una pista visual, no
// bloquea nada en JS. Un precio negativo se usa tal cual al descontar
// un consumo (`precio * cantidad`), lo que factura EN CONTRA del
// huésped en vez de cobrarle. Ahora se valida explícito en los dos
// lugares (se permite 0, se bloquea negativo).
  ...
  if (permitido) {
    elemento.querySelectorAll('.btn-guardar-producto').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        // (209 / auditoría H21) Bloquea negativos en precio/cantidad
        // estándar — se permite 0.
        if ((payload.precio != null && payload.precio < 0) || (payload.cantidad_estandar != null && payload.cantidad_estandar < 0)) {
          mostrarToast('El precio y la cantidad estándar no pueden ser negativos.', 'error');
          return;
        }
    ...
    elemento.querySelector('#form-nuevo-producto').addEventListener('submit', async (e) => {
      ...
      const precioNuevo = Number(form.get('precio'));
      const cantidadEstandarNueva = form.get('cantidad_estandar') ? Number(form.get('cantidad_estandar')) : null;
      // (209 / auditoría H21) Mismo bloqueo que en la edición inline.
      if (precioNuevo < 0 || (cantidadEstandarNueva != null && cantidadEstandarNueva < 0)) {
        mostrarToast('El precio y la cantidad estándar no pueden ser negativos.', 'error');
        return;
      }
      const { error } = await supabase.from('minibar_productos').insert({
        ...
        precio: precioNuevo,
        cantidad_estandar: cantidadEstandarNueva,
